...
UBUNTU_IMAGE = 'Linux Ubuntu 16.04 LTS 64-bit'
...
class CloudStackVirtualMachine(virtual_machine.BaseVirtualMachine):
  ...
  def _CreateDependencies(self):
    ...
    with open(self.ssh_public_key) as keyfd:
        ...
        if not self.cs.get_ssh_keypair(self.ssh_keypair_name):
  ...
  def _DeleteDependencies(self):
    ...
    # Remove the keypair
    if self.cs.get_ssh_keypair(self.ssh_keypair_name):
        self.cs.unregister_ssh_keypair(self.ssh_keypair_name)
  ...
  def _Create(self):
    ...
    template = self.cs.get_template(self.image)
  ...
  def _PostCreate(self):
    ...
    self.ip_address = network_interface['ipaddress']
  ...
  def _Exists(self):
    ...
    # Check if VM exisits
    vm = self.cs.get_virtual_machine(self.name)
    if vm and 'id' in vm:
        return True
